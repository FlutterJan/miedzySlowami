...
class Opracowanielekturylaptop extends StatefulWidget {
  final int nrLektury;
  const Opracowanielekturylaptop({super.key, required this.nrLektury});

  @override
  _OpracowanielekturylaptopState createState() =>
      _OpracowanielekturylaptopState();
}

class _OpracowanielekturylaptopState extends State<Opracowanielekturylaptop> {
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          ...
                          _buildExpansionTile('Geneza', row[2].toString(),width),
                          _buildExpansionTile('Gatunek', row[3].toString(),width),
                          _buildExpansionTile(
                            'Czas i miejsce akcji',
                            row[4].toString(),width
                          ),
                          _buildExpansionTile('Bohaterowie', row[5].toString(),width),
                          _buildExpansionTile('Streszczenie', row[6].toString(),width),
                    
                          if (row[7].toString() != "-")
                            _buildExpansionTile('Pytania Jawne', row[7].toString(),width),
                        ],
                      ),
                    ),
                  ),
